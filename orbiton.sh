...
show_help() {
  echo -e "\n${BLUE}${BOLD}🪐 Orbiton System CLI Manager (v1.35.0)${NC}"
  echo -e "Usage: ${GREEN}sudo orbiton <command|flag> [args]${NC}\n"
  echo -e "${BOLD}Commands & Short Flags:${NC}"
  ...
  echo -e "\n${BOLD}Examples:${NC}"
  echo -e "  ${BLUE}sudo orbiton start${NC}"
  echo -e "  ${BLUE}sudo orbiton -i${NC}"
  echo -e "  ${BLUE}sudo orbiton create-admin admin mypassword${NC}\n"
  echo -e "Created by ${YELLOW}iamprmgvyt${NC}\n"
}
...
EOF
    ...
    ;;
  version)
    echo -e "Orbiton Orchestrator v1.34.0"
    ;;
  *)
    show_help
    ;;
